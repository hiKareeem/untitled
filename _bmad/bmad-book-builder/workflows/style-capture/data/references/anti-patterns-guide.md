...
### Detection Procedure
1. Scan for cliche patterns
2. For each cliche detected, extract example
...
```yaml
cliche_phrases:
- phrase: "[cliche expression]"
    examples:
- original: "[sentence with cliche]"
        alternative: "[fresh alternative]"
```

### Minimum Detection
3 cliches (or note "few cliches detected - original language used")

### Note
Use web search to verify if phrases are common cliches if uncertain.
...
21. **Cliche phrases** - Overused expressions
...
3. **Cliche Phrases** (Medium impact, creative)
