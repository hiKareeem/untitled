...
- **B2 Ch 38:** Saturation event (~10 months post-BLACKWEIR). The Cathedral's broadband dephasing architecture fails at global saturation scale — insufficient, not broken. "The banks were still there. The river had been larger than the banks could hold." The stone conducts the global composite through Nephthys's branded palm. She falls during morning practice session, seizes (12 seconds), loses consciousness. Congregation contracts: 3 of 15 leave during the event; 12 remain. The Cathedral's protective properties are confirmed as physics-limited — they resist local and regional signal density but cannot reduce a simultaneous global output. Imani manages the aftermath in the nave. Nephthys wakes, speaks "Circulation" aloud for the first time, delivers the framework to the twelve in the side chapel.

**Significance:** Protects Nephthys during BLACKWEIR — not by divine intervention but by physics. She interprets it as providence. This becomes the foundation of a theology that reshapes the world. First congregation forms here among survivors. The Cathedral's survival pocket (~340 people) is confirmed by AEGIS orbital data (Book 2 AEGIS chapter). **B2 Ch 38 update:** The dephasing architecture has a confirmed limit — global simultaneous saturation exceeds its capacity. The Cathedral is not an unconditional sanctuary.

**Current State:** Standing. Interior occupied by ~12 core congregants (post-saturation contraction from 15; down from 60+ peak). Dephasing architecture intact but confirmed insufficient at global saturation scale.
...
- **B3 Ch 31:** Contact Protocols Working Group convenes in Committee Chamber 4 (smaller than Chamber 3) to assess aggregate communication feasibility. Zeyad observes from gallery (third row, eastern section) over three sessions. Session 1: terms of reference (three hours). Session 2: 340-page compendium review, Petrov's question ("Has the Working Group considered the possibility that the aggregate is not a party?"). Session 3: Working Group finding — aggregate is condition, not party. No counterparty. Cannot negotiate with state of being. Can only adjust parameters that produce it. Dr. Vasquez (Chilean diplomat, chair) presents finding. Pacific Rim delegate independently uses "replacement framework" language. Finding filed alongside Zeyad's replacement document (submitted B3Ch19, tabled, not read). Neither produces action. Zeyad does not stop at Khalil's portrait (eastern alcove). Grey pass degraded (typography blurred, edges lost geometry). Pen has not written since B3Ch19. Notebook carried but not opened. Jordanian flag worse (lower half static, upper half holding). Hum in foundation (post-Voice texture, aggregate precursor). Building's acoustic dampening system filters aggregate output while Working Group discusses it. New interior established: **Committee Chamber 4** (smaller than Chamber 3, 14 delegate seats, provisional status).
