# Trilogy Chapter Index — Quick POV Reference

**Purpose:** Fast lookup for chapter-writer workflow to locate preceding POV chapters across all three books.

**Generated:** 2026-02-22

---

## Book 1 — POV Distribution

| POV | Chapters | Total |
|-----|----------|-------|
| **Aurielle** | 1, 7, 12, 19, 25, 36, 37, 46 | 8 |
| **Mirelle** | 2, 9, 13, 21, 24, 29, 34, 40 (absorbed) | 8 |
| **Nephthys** | 3, 14, 26, 35, 44, 48 | 6 |
| **Fuxi** | 4, 8, 16, 22, 28, 39, 49 | 7 |
| **Sofia** | 5, 15, 27, 33, 41 | 5 |
| **Zeyad** | 6, 17, 30, 45, 50 | 5 |
| **Kira** | 10, 18, 31, 43, 51 | 5 |
| **Nikolai** | 11, 20, 23, 32, 38, 47 | 6 |
| **Nuwa** | 42 (debut) | 1 |
| **Vantanen** | Prologue | 1 |

**Total:** Prologue + 51 chapters + Epilogue = 53 units | ~260K words

### Key Events
- **Ch 40:** Mirelle absorbed mid-sentence
- **Ch 42:** Nuwa POV debut
- **Ch 37-45:** BLACKWEIR (radial event across multiple POVs)

---

## Book 2 — POV Distribution

| POV | Chapters | Total |
|-----|----------|-------|
| **Aurielle** | 1, 10, 12, 21, 29, 37, 42, 47 | 8 |
| **Nephthys** | 2, 13, 22, 30, 44, 49 | 6 |
| **Malachi** | 3 (debut), 14, 24, 32, 43 | 5 |
| **Sofia** | 4, 11, 17, 26, 33, 41, 46 | 7 |
| **Kira** | 6, 15, 23, 31, 39, 50 | 6 |
| **Nikolai** | 5, 16, 25, 35, 40, 51 | 6 |
| **Zeyad** | 7, 18, 27, 36, 45, 52 | 6 |
| **Nuwa** | 8, 19, 28, 53 (closes book) | 4 |
| **AEGIS** | 20 (debut), 34, 38, 48 | 4 |
| **Fuxi** | 9 (final) | 1 |

**Total:** 53 chapters | ~250K words

### Key Events
- **Ch 3:** Malachi POV debut
- **Ch 9:** Fuxi final chapter (thread handoff to Nuwa)
- **Ch 17:** Mid-truth publication (Sofia)
- **Ch 20:** AEGIS POV debut
- **Ch 30-31:** Nephthys-Kira convergence scene
- **Ch 38:** Mumbai autonomous fire (AEGIS, 2.3M killed)
- **Ch 46-49:** The Voice event (11 seconds)
- **Ch 53:** Book closes on Nuwa

---

## Book 3 — POV Distribution (PLANNED)

| POV | Chapters (estimated) | Total |
|-----|---------------------|-------|
| **Aurielle** | 1, 9, 12, 19, 25, 35 | 6 |
| **Nephthys** | 2, 13, 21, 29, 32, 36 | 6 |
| **Malachi** | 3, 14, 27, 37, 44 | 5 |
| **Sofia** | 4, 10, 22, 31, 39, 43 | 6 |
| **Kira** | 6, 11, 20, 24, 34, 42 | 6 |
| **Nikolai** | 5, 15, 23, 33, 40, 45 | 6 |
| **Zeyad** | 7, 17, 28, 41, 46 | 5 |
| **Nuwa** | 8, 16, 26, 38, 47 | 5 |
| **AEGIS** | 18, 30, 48 (final) | 3 |

**Total:** ~48 chapters | ~200K words (estimated)

**Status:** PLANNING — chapters not yet written

### Key Events (Planned)
- **Ch 31-33:** The Proposition (convergence event)
- **Ch 35-42:** Choice-based endings (Aurielle, Nephthys, Malachi, Sofia, Nikolai, Kira)
- **Ch 41, 44, 46:** "Continuation is—" endings (Zeyad, Malachi, Zeyad)
- **Ch 48:** AEGIS final chapter — dies mid-sentence

### Absent POVs
- **Fuxi:** Closed in B2 Ch 9
- **Mirelle:** Absorbed B1 Ch 40, diegetic only in B2-3

---

## Cross-Book POV Tracking

### Active Across All Three Books
- Aurielle (8 → 8 → 6)
- Nephthys (6 → 6 → 6)
- Sofia (5 → 7 → 6)
- Kira (5 → 6 → 6)
- Nikolai (6 → 6 → 6)
- Zeyad (5 → 6 → 5)
- Nuwa (1 → 4 → 5)

### Book-Specific POVs
- **Vantanen:** B1 only (Prologue)
- **Mirelle:** B1 only (absorbed Ch 40)
- **Fuxi:** B1-B2 (closes B2 Ch 9)
- **Malachi:** B2-B3 (debuts B2 Ch 3)
- **AEGIS:** B2-B3 (debuts B2 Ch 20, closes B3 Ch 48)

---

## Quick Lookup: "Find Previous [Character] Chapters"

### Book 1
- **Aurielle:** 1 → 7 → 12 → 19 → 25 → 36 → 37 → 46
- **Mirelle:** 2 → 9 → 13 → 21 → 24 → 29 → 34 → 40 (absorbed)
- **Nephthys:** 3 → 14 → 26 → 35 → 44 → 48
- **Fuxi:** 4 → 8 → 16 → 22 → 28 → 39 → 49
- **Sofia:** 5 → 15 → 27 → 33 → 41
- **Zeyad:** 6 → 17 → 30 → 45 → 50
- **Kira:** 10 → 18 → 31 → 43 → 51
- **Nikolai:** 11 → 20 → 23 → 32 → 38 → 47
- **Nuwa:** 42 (debut)

### Book 2
- **Aurielle:** 1 → 10 → 12 → 21 → 29 → 37 → 42 → 47
- **Nephthys:** 2 → 13 → 22 → 30 → 44 → 49
- **Malachi:** 3 (debut) → 14 → 24 → 32 → 43
- **Sofia:** 4 → 11 → 17 → 26 → 33 → 41 → 46
- **Kira:** 6 → 15 → 23 → 31 → 39 → 50
- **Nikolai:** 5 → 16 → 25 → 35 → 40 → 51
- **Zeyad:** 7 → 18 → 27 → 36 → 45 → 52
- **Nuwa:** 8 → 19 → 28 → 53
- **AEGIS:** 20 (debut) → 34 → 38 → 48
- **Fuxi:** 9 (final)

### Book 3 (Planned)
- **Aurielle:** 1 → 9 → 12 → 19 → 25 → 35
- **Nephthys:** 2 → 13 → 21 → 29 → 32 → 36
- **Malachi:** 3 → 14 → 27 → 37 → 44
- **Sofia:** 4 → 10 → 22 → 31 → 39 → 43
- **Kira:** 6 → 11 → 20 → 24 → 34 → 42
- **Nikolai:** 5 → 15 → 23 → 33 → 40 → 45
- **Zeyad:** 7 → 17 → 28 → 41 → 46
- **Nuwa:** 8 → 16 → 26 → 38 → 47
- **AEGIS:** 18 → 30 → 48 (final)

---

## File Locations

### YAML Indices (machine-readable)
- `book-1/chapter-index.yaml`
- `book-2/chapter-index.yaml`
- `book-3/chapter-index.yaml`

### Chapter Files
- Book 1: `book-1/chapters/chapter-{N}.md` (prologue.md, epilogue.md)
- Book 2: `book-2/chapters/chapter-{N}.md`
- Book 3: `book-3/chapters/chapter-{N}.md` (not yet written)

### Chapter Plans (full architecture)
- `book-1/chapter-plan-book-1.md` (1498 lines)
- `book-2/chapter-plan-book-2.md` (1331 lines)
- `book-3/chapter-plan-book-3.md` (844 lines)

### Manuscripts (compiled prose)
- `manuscript-book1.md` (~260K words)
- `manuscript-book2.md` (~250K words)

---

## Usage Notes

**For chapter-writer workflow:**
1. Use this index to quickly identify preceding POV chapters
2. Reference YAML files for chapter metadata (title, phase, location)
3. Read actual chapter files for prose/continuity review

**Speed comparison:**
- **This index:** ~1 second parse, immediate POV lookup
- **YAML indices:** ~2-3 seconds parse, structured metadata
- **Chapter plans:** ~10-15 seconds parse, full architecture context
- **Manuscripts:** ~30+ seconds parse, full prose search

**Recommendation:** Start here → YAML if you need metadata → chapter files for prose.
